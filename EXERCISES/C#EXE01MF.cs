...
namespace Mathmatical_Formulas
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            double HalfCir = ( sideA + sideB + sideC) /2;
            // Find the area 
            double TriangleArea = Math.Sqrt(HalfCir) * (HalfCir - sideA) * (HalfCir - sideB) * (HalfCir - sideC);
            Console.WriteLine($" P = {HalfCir}  Area = {TriangleArea} ");
